...
In order to execute the test cases, first ensure that at least one test case is created within the testCase files for an existing driver.  Then run the runAllTestCases.py within the testCase Executables.  The output will be sent to an HTML file and then opened in the browser. 
